...
class AudioQueue {
  ...
  async addChunk(base64Audio, chunkId = null) {
    try {
      console.log('🔍 DEBUG: AudioQueue.addChunk called with chunkId:', chunkId);
      console.log('🔍 DEBUG: Audio data length:', base64Audio?.length || 'undefined');
      console.log('🔍 DEBUG: Currently playing?', this.isPlaying);
      
      const audioFile = await this.convertBase64ToAudio(base64Audio, chunkId);
      console.log('🔍 DEBUG: convertBase64ToAudio returned:', audioFile);
      ...
      if (!this.isPlaying) {
        console.log('🔍 DEBUG: Not playing, calling playAudio immediately');
        await this.playAudio(audioItem);
      } else {
        console.log('🔍 DEBUG: Currently playing, adding to queue. Queue length:', this.queue.length);
  ...
  async convertBase64ToAudio(base64String, chunkId = null) {
    try {
      console.log('🔍 DEBUG: convertBase64ToAudio called with chunkId:', chunkId);
      console.log('🔍 DEBUG: Base64 string length:', base64String?.length || 'undefined');
      ...
      console.log('🔍 AUDIO: Testing document directory access...');
      const dirInfo = await FileSystem.getInfoAsync(FileSystem.documentDirectory);
      console.log('🔍 AUDIO: Document directory info:', dirInfo);
      const filename = `audio_chunk_${chunkId || Date.now()}_${Math.random().toString(36).substr(2, 9)}.wav`;
      const fileUri = `${FileSystem.documentDirectory}${filename}`;
      console.log('🔍 DEBUG: Creating file at:', fileUri);
      ...
      console.log('🔍 DEBUG: File write completed, checking if file exists...');
      ...
      console.error("🚨 This is likely why audio is silent!");
      throw error;
    }
  }
  ...
  async playAudio(audioItem) {
    try {
      console.log('🔍 DEBUG: playAudio called with audioItem:', audioItem);
      this.isPlaying = true;
      ...
      if (AudioOutputManager.isAvailable()) {
        try {
          ...
          console.log('🔊 Forced speaker output before audio playback');
        } catch (error) {
      ...
      if (this.onChunkStarted && audioItem.chunkId) {
        console.log('🔍 DEBUG: Calling onChunkStarted for chunk:', audioItem.chunkId);
        this.onChunkStarted(audioItem.chunkId);
      }
      
      console.log('🔍 DEBUG: About to call Audio.Sound.createAsync with URI:', audioItem.uri);
      console.log('🔍 DEBUG: File exists check before playback...');
      try {
        const fileExists = await FileSystem.getInfoAsync(audioItem.uri);
        console.log('🔍 DEBUG: File exists result:', fileExists);
        if (fileExists && !fileExists.exists) {
          console.warn('⚠️ File check says file does not exist, but trying anyway');
        }
      } catch (error) {
        console.warn('⚠️ File existence check failed, but continuing with playback:', error);
      }      
      ...
      console.log('🔍 DEBUG: Audio.Sound.createAsync SUCCESS! Sound object:', !!sound);
      ...
      sound.setOnPlaybackStatusUpdate((status) => {
        ...
        console.log('🔍 DEBUG: Playback status update:', status.isLoaded, status.isPlaying, status.didJustFinish);
        if (status.didJustFinish) {
          this.playNext();
        }
      });
      
      console.log('🎵 AUDIO PLAYBACK STARTED SUCCESSFULLY for chunk:', audioItem.chunkId);
      ...
      console.error("🚨 THIS IS THE AUDIO PLAYBACK FAILURE! ☝️");
      console.error('🔍 DEBUG: playAudio failed at Audio.Sound.createAsync');
      this.isPlaying = false;
  ...
  async cleanupTempFile(fileUri) {
    try {
      ...
      if (fileExists.exists) {
        ...
        console.log('🗑️ Cleaned up temp audio file:', fileUri);
      }
