    ...
    if (url) {
      ...
      if (hashIndex !== -1) {
        ...
        if (accessToken) {
          ...
          try {
            ...
            // So create user profile for verified OAuth users
            if (data?.user?.id && data?.user?.email_confirmed_at) {
              try {
                await ensureUserProfile(data.user.id);
                console.log('✅ Profile created for OAuth user');
              } catch (profileError) {
                console.error('⚠️ Profile creation failed for OAuth user:', profileError.message);
      ...
      if (code) {
        ...
        try {
          ...
          // OAuth providers (Google, Apple) automatically verify emails
          // So create user profile for verified OAuth users
          if (codeData?.user?.id && codeData?.user?.email_confirmed_at) {
            try {
              await ensureUserProfile(codeData.user.id);
              console.log('✅ Profile created for OAuth user');
            } catch (profileError) {
              console.error('⚠️ Profile creation failed for OAuth user:', profileError.message);
              // Continue anyway - profile can be created later
