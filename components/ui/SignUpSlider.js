...
import { View, Animated, PanResponder, Text, Pressable, StyleSheet } from 'react-native';
...
export default function SignUpSlider({ isOpen, onClose, navigation }) {
  // Get safe area insets to handle notches and home indicators
  const insets = useSafeAreaInsets();
  
  // Create animation value - starts at 1 (hidden) when closed, 0 (visible) when open
  ...
  // Handle animation when isOpen prop changes
  useEffect(() => {
    Animated.spring(slideAnim, {
      toValue: isOpen ? 0 : 1,  // Animate to 0 when opening, 1 when closing
      useNativeDriver: true,    // Use native driver for better performance
      tension: 20,              // Lower tension for smoother animation
      friction: 8               // Lower friction for more bounce
    }).start();
  }, [isOpen]);

  // Setup pan (drag) gesture handler
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        const newValue = gestureState.dy / 100;  // Match the new slide distance
        slideAnim.setValue(Math.max(0, Math.min(1, newValue)));
      },

      // Handle drag release
      onPanResponderRelease: (_, gestureState) => {
        // If dragged down more than 50px, close the slider
  ...
  // Convert animation value to Y position
  const translateY = slideAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 400],  // Increased from 200 to 400 to ensure it slides down enough
  });
  ...
  return (
    <View style={[
      styles.container,
      {
        bottom: -insets.bottom, // Account for safe area
        zIndex: isOpen ? 1 : 0,
      ...
      >
        ...
        <View style={styles.content}>
          ...
          <Pressable 
            ...
            style={[styles.signUpButton, styles.emailButton]}
...
const styles = StyleSheet.create({
  ...
  slider: {
    backgroundColor: '#404040', // neutral-800
